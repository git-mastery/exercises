...
MERGE_NOT_UNDONE = (
    "The merge commit 'Introduce others' is still present on the main branch. "
    "You need to undo the merge so that only Rick and Morty remain on main."
)
...
def verify(exercise: GitAutograderExercise) -> GitAutograderOutput:
    ...
    if any("Introduce others" in msg for msg in commit_messages_in_main):
    ...
    if len(commit_messages_in_main) != 2 or not (has_rick and has_morty):
        raise exercise.wrong_answer([MAIN_COMMITS_INCORRECT])

    # Check that the merge commit is not present on others
    if any("Introduce others" in msg for msg in commit_messages_in_others):
        raise exercise.wrong_answer([MERGE_NOT_UNDONE])
    ...
    if len(commit_messages_in_others) != 3 or not (has_birdperson and has_cyborg and has_tammy):
